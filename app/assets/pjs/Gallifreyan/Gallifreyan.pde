...
void writeSentence(int type) {
  ...
  for (int i=0;i<sentence.length;i++) {
    wordRadius=append(wordRadius, constrain(sentenceRadius*sentence[i].length/charCount*1.2, 0, sentenceRadius/2));
  ...
  for (int i=0;i<sentence.length;i++) {
    ...
    int index = round(map(angle1, 0, TWO_PI, 0, float(sentence[i].length)));
  ...
  for (int i=0;i<sentence.length;i++) {
    pos=PI/2;
    float letterRadius = wordRadius[i]/(sentence[i].length+1)*1.5;
    for (int j=0;j<sentence[i].length;j++) {
      if(apostrophes[i][j]){
        float a=pos+PI/sentence[i].length-0.1;
        float d=0;
        float tempX=x[i];
        float tempY=y[i];
        ...
        a=pos+PI/sentence[i].length+0.1;
        d=0;
        tempX=x[i];
        tempY=y[i];
        while (pow (tempX-width/2, 2)+pow(tempY-(height/2 - 100), 2)<pow(sentenceRadius+20, 2)) {
      ...
      if (vowel) {
        ...
        if (sentence[i][j].charAt(0)=="b"||sentence[i][j].charAt(0)=="#"||sentence[i][j].charAt(0)=="d"||sentence[i][j].charAt(0)=="f"||sentence[i][j].charAt(0)=="g"||sentence[i][j].charAt(0)=="h") {
          ...
          for (int k=0;k<lines;k++) {
            lineX=append(lineX, tempX);
            lineY=append(lineY, tempY);
            arcBegin=append(arcBegin, pos+0.5);
        ...
        if (sentence[i][j].charAt(0)=="j"||sentence[i][j].charAt(0)=="k"||sentence[i][j].charAt(0)=="l"||sentence[i][j].charAt(0)=="m"||sentence[i][j].charAt(0)=="n"||sentence[i][j].charAt(0)=="p") {
          ...
          for (int k=0;k<lines;k++) {
            ...
            lineRad=append(lineRad, letterRadius*1.9);
          }
          if (sentence[i][j].length()>1) {
            int vowelIndex=1;
            if (sentence[i][j].charAt(1)=="@") {
          ...
          if (sentence[i][j].length()>1) {
            if (sentence[i][j].charAt(1)=="@") {
              if (nested[i][j]) {
                ...
                makeArcs(tempX, tempY, x[i], y[i], wordRadius[i], letterRadius*1.8, pos+TWO_PI, pos-TWO_PI);
              }
            }
          }
        }
...
void makeDots(float mX, float mY, float r, int amnt, float pos, float scaleFactor) {
  noStroke();
  fill(fg);
  if (amnt==3) {
    ellipse(mX+cos(pos+PI)*r/1.4, mY+sin(pos+PI)*r/1.4, r/3*scaleFactor, r/3*scaleFactor);
  }
  ellipse(mX+cos(pos+PI+scaleFactor)*r/1.4, mY+sin(pos+PI+scaleFactor)*r/1.4, r/3*scaleFactor, r/3*scaleFactor);
  ellipse(mX+cos(pos+PI-scaleFactor)*r/1.4, mY+sin(pos+PI-scaleFactor)*r/1.4, r/3*scaleFactor, r/3*scaleFactor);
  noFill();
  stroke(fg);
}
